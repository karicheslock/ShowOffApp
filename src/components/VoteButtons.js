...
function VoteButtons({ userCollection }) {
    ...
    const handleClick = async (type) => {
        let upVotesUsers = userCollection.upVotesUsers;
        let downVotesUsers = userCollection.downVotesUsers;
        ...
        if (type === 'upvote') {
            await updateDoc(userCollectionRef, {
                upVotesUsers: toggleVote ? arrayRemove(userId) : arrayUnion(userId)
            })
        } else {
            await updateDoc(userCollectionRef, {
                downVotesUsers: toggleVote ? arrayRemove(userId) : arrayUnion(userId)
            })
        }

        setToggleVote((toggleVote) => !toggleVote);
    ...
    <div>
        <div className='flex mb-4'>
            <button onClick={() => handleClick('upvote')} className="mr-2">
                { toggleVote ? <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512" className='fill-emerald-600 w-6'>
        ...
        <div className='flex'>
            <button onClick={() => handleClick('downvote')} className="mr-2">
                { toggleVote ? <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512" className='fill-rose-400 w-6'>
